...
$scope.getWorkorderByStatusEmployeeKey = function(employeekey,workstatuskey){
  ...
  $scope.today_DT = convert_DT(new Date());
  // $ionicLoading.show();
  $scope.myPromise =  $http.get(HOSTNAME + "/getWorkorderByStatusEmployeeKey?employeekey=" + employeekey+"&workstatuskey="+workstatuskey+ "&today="+$scope.today_DT+"&userKey="+$scope.toServeremployeekey+"&OrganizationID="+$scope.OrganizationID)//  status /roomtype  /employee
            .success(function(response)
            {
              $scope.scrollToTop();
              // $ionicLoading.hide();
