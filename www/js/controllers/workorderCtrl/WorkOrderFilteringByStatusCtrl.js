...
$scope.getWorkorderBYStatusKey =function(statuskey){
    ...
    // alert(statuskey);
    $scope.today_DT = convert_DT(new Date());
   $scope.myPromise = $http.get(HOSTNAME+ "/workorderFilterByStatusEmpView?statuskey=" + statuskey+"&today="+$scope.today_DT+"&employeekey="+$scope.toServeremployeekey+"&OrganizationID="+$scope.OrganizationID)//  status /roomtype  /employee
            .success(function(response)
            {
                // $ionicLoading.hide();
               // debugger;
