    ...
    params : {'fileName': filename}
            ...
            $cordovaFileTransfer.upload(url, targetPath, options).then(function(result) {
                      ...
                      if(compare === '"File is uploaded"'){
                              ...
                              $scope.myPromise =  $http.post(HOSTNAME+"/pho1_Ang6",$scope.uploadPhoto)
                                       .then(function(resp)
                                      {
                              ...
                              var s=t.getSeconds();
                               ...
                               var complete_Time="";
                                 complete_Time= $scope.today_DT+" "+h+":"+mi+":"+s;
                                        ...
                                        var type = 'automatic';
                                        // console.log(type);
                                       $scope.myPromise =  $http.get(HOSTNAME+"/barcodeRoom_Ang?wkey=" + workorder_Key+"&barcode="+$scope.scannedBarcode+"&updatetype="+type+"&employeekey="+$scope.toServeremployeekey+"&OrganizationID="+$scope.OrganizationID+'&complete_Time='+complete_Time)
...
//..........For Scanned workorder.....start....
  ...
  $ionicPlatform.ready(function() {
          //alert('Platform ready');
           ...
           $scope.myPromise =  $http.get(HOSTNAME+"/scanforWorkorder_emp?barcode="+$scope.inbarcode+"&empkey="+$scope.toServeremployeekey+"&ondate="+ondate+"&OrganizationID="+$scope.OrganizationID)
                .success(function(response)
                {
      ...
      { text: 'Cancel' },
      {
        ...
        onTap: function(e) {
          if (!$scope.data.barcodeValue) {
                ...
                $scope.myPromise =  $http.get(HOSTNAME+"/scanforWorkorder_emp?barcode="+$scope.data.barcodeValue+"&empkey="+$scope.toServeremployeekey+"&ondate="+ondate+"&OrganizationID="+$scope.OrganizationID)
                .success(function(response)
                {
                  // console.log("response.length"+response.length);
                  if(response.length >0){
      ...
      console.log("continueCurrentWorkOrder in delay "+WorkorderKey);
        ...
        $scope.myPromise =  $http.get(HOSTNAME+"/continueCurrentWorkOrder?WorkorderKey="+WorkorderKey+"&emp="+$scope.toServeremployeekey+"&OrganizationID="+$scope.OrganizationID)
                .success(function()
                {
                    var alertPopup = $ionicPopup.alert({
                      title: 'Delay',
                      template: 'Work order has been continued!'
                      });
                      $timeout(function() {
                                $ionicLoading.hide();
                                      alertPopup.close();
