  ...
  $scope.managerInspection = false;
            $scope.doRefresh = function(){ //for supervisor only
                ...
                if($scope.toServeremployeekey){
                    // $ionicLoading.show();
                     $scope.myPromise =   $http.get(HOSTNAME+ "/getSupervisorInspectionView?to_date=" + todat_date+"&employeekey="+$scope.toServeremployeekey+"&OrganizationID="+$scope.OrganizationID)
